...
public class GraphAlgorithms {
     ...
     */
    public static <V, E> LinkedList<V> BreadthFirstSearch(Graph<V, E> g, V vOrig) {
        if (!g.validVertex(vOrig)) {
            return null;
        }
        boolean visitantes[] = new boolean[g.numVertices()];
        LinkedList<V> qbfs = new LinkedList<>();
        LinkedList<V> qaux = new LinkedList<V>();
        qbfs.add(vOrig);
        qaux.add(vOrig);

        visitantes[g.getKey(vOrig)] = true;
        while (!qaux.isEmpty()) {
            vOrig = qaux.remove();
            for (V vAdj : g.adjVertices(vOrig)) {
                if (!visitantes[g.getKey(vAdj)]) {

                    qbfs.add(vAdj);
                    qaux.add(vAdj);
                    visitantes[g.getKey(vAdj)] = true;
                }
            }
        }
        return qbfs;
     ...
     */
    private static <V, E> void getPath(Graph<V, E> g, V vOrig, V vDest, V[] verts, int[] pathKeys,
            LinkedList<V> path) {
